...
function parseComponents( results, tmpl, targetDirectory, renderers ){
    ...
    var definitions = flow(
        renderMd( md ),
        renderHtml( html ),
        renderJs( js, targetDirectory, renderers ),
        reduce(
...
function mapComponents( componentsDirectory, tmpl, targetDirectory, renderers ){
    return new Promise(
        ( resolve, reject ) => Promise
            .all( [
            ...
            .then(
                ( results ) => resolve(
                    parseComponents( results, tmpl, targetDirectory, renderers )
                )
            )
            .catch( reject )
    );
...
module.exports = function renderComponents( componentsDirectory, targetDirectory, renderers ){
